...
public class FXMLDocumentController implements Initializable {
    ...
    void ActionOnValidateStudent(ActionEvent event) throws SQLException {
        ...
        if ((idMatcher == Long.valueOf(studetIdTovalidate)) && isVoted()) {
            ...
            GetMessegeAsAlert.getCondidatesNotFoundWarning("Sorry, you can't cast vote, You have already voted");
        }
    ...
    private  void insertVotedCondidateDetails() throws SQLException {
     ...
     String Name_candidate=umidwarData[0];
     String Voter_id=umidwarData[1];
     String detailsOfVoter[]=dataAboutCandidate.split(",");
     System.out.println("Voted_Candidate: "+Name_candidate+" Candidate_ID:"+Voter_id+" Voter_Id:"+detailsOfVoter[0]+" voterName:"+detailsOfVoter[1]
     ...
     CurdOprationAgainstDatabase.insertDataToDatabase("insert into votedCandidateDetails(votedCandidate,votedCandidateId,voterId,voterName,voterDept,voterGender) "
             +" values('"+Name_candidate+"','"+Voter_id+"','"+detailsOfVoter[0]+"','"+detailsOfVoter[1]+"','"+detailsOfVoter[2]+"','"+detailsOfVoter[3]+"')");
    }
    @FXML
    void ActionOnResultShow(ActionEvent event) throws SQLException {
        ...
        Eirst_WinnerID.setText("WINNER:"+FirstWinnerName);
        while(rs2.next())
        {
            SecondWinnerName=rs2.getString("candidate_name");
            
        }
        Second_WinnerID.setText("1ST Runnerup:"+SecondWinnerName);
